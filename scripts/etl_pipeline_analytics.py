...
WEATHER_SCHEMA = pa.DataFrameSchema({
    ...
    # coerce=True safely handles type casting issues from raw CSVs
    "tmax": pa.Column(float, checks=pa.Check.in_range(-5.0, 55.0), coerce=True), 
    "hrMin": pa.Column(float, checks=pa.Check.in_range(0.0, 100.0), coerce=True), 
})
...
def process_tourism_data(raw_path: Path, files: list) -> pd.DataFrame:
    ...
    if not dfs_list:
        raise ValueError("No valid ISTAC data collected. Pipeline halting.")
...
def run_pipeline():
    ...
    logger.info("STARTING DATA PIPELINE (v2.2 - Windows Compatibility & Strict Contracts)")
